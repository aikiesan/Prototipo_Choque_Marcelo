...
def main():
    ...
    with col_mapa:
        ...
        if map_data:
            with st.expander("🔧 Debug - Dados do Mapa (temporário)", expanded=False):
                ...
                if map_data.get('last_object_clicked_tooltip'):
                    tooltip_debug = map_data.get('last_object_clicked_tooltip')
                    st.write("**Tooltip clicado:**", tooltip_debug)
                    st.write("**Tipo do tooltip:**", type(tooltip_debug))
                    if isinstance(tooltip_debug, str):
                        st.write("**String limpa:**", repr(tooltip_debug))
        ...
        if map_data and map_data.get('last_object_clicked_tooltip'):
            ...
            if tooltip_data:
                # Tentar múltiplas formas de extrair o nome da região
                if isinstance(tooltip_data, dict):
                    nova_regiao = tooltip_data.get('Região:')
                elif isinstance(tooltip_data, str):
                    # Se for string, pode estar no formato "Região: Nome da Região"
        ...
        # Debug: Mostrar região extraída
        if nova_regiao:
            st.info(f"🔧 DEBUG: Região extraída = '{nova_regiao}' | Atual = '{st.session_state.regiao_ativa}'")
